...
const SellHouseCard: React.FC<CardProps> = ({ id, image, details, location, description, bedroom, bathroom, area_square, garage, time_since_upload }) => {
    ...
    return (
        <div className="max-w-sm rounded overflow-hidden text-center shadow-lg">
            ...
            <div className="px-6 pb-2">
                ...
                <p className=' text-black'> {time_since_upload} ago</p>
                <Link href={`/pages/SellHouseDetail/${id}` }>
...
export  const CardList: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await axios.get('http://127.0.0.1:8000/SellHouse/');
                setCards(res.data);
            } catch (error) {
